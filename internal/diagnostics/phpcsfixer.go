...
// Format implements the FormattingProvider interface
func (dp *PhpCsFixer) Format(filePath string, content string) (string, error) {
	...
	log.Printf("%s%s Running php-cs-fixer command: %s", logging.LogTagLSP, logging.LogTagServer, cmd)
	log.Printf("%s%s Content length being sent: %d bytes", logging.LogTagLSP, logging.LogTagServer, len(content))
	...
	log.Printf("%s%s php-cs-fixer output length: %d bytes", logging.LogTagLSP, logging.LogTagServer, len(diffOutput))
	if len(diffOutput) > 0 {
		log.Printf("%s%s php-cs-fixer output preview (first 200 chars): %s", logging.LogTagLSP, logging.LogTagServer,
			func() string {
